...
class Audio2landmark_Dataset(data.Dataset):
    ...
    def __init__(self, dump_dir, dump_name, num_window_frames, num_window_step, status):
        ...
        print(os.path.join(self.dump_dir, '{}_{}_au.pickle'.format(dump_name, status)))
        print(os.path.join(self.dump_dir, '{}_{}_fl.pickle'.format(dump_name, status)))

        valid_idx = list(range(len(self.au_data)))
        print(f'au_len : {len(self.au_data)}')
        print(f'fl_len : {len(self.fl_data)}')
        random.seed(0)
...
class Audio2landmark_Dataset_array(data.Dataset):
    ...
    def my_collate_in_segments_noemb(self, batch):
        fls, aus = [], []
        for fl, au in batch:
            fl_data, au_data = fl[0], au
            print(fl_data.shape[0], au_data.shape[0])
            assert (fl_data.shape[0] == au_data.shape[0])
