...
def pySTFT_len(x, length, fft_length=1024):
    # print(x.shape)
    x = np.pad(x, int(fft_length // 2), mode='reflect')
    # print(x.shape)
    # print(x.shape[-1])
...
def extract_f0_func_audiofile_len(base, audio, gender='M'):
    floor_sp, ceil_sp = -80, 30
    mel_basis = mel(16000, 1024, fmin=90, fmax=7600, n_mels=80).T
    min_level = np.exp(-100 / 20 * np.log(10))
    b, a = butter_highpass(30, 16000, order=5)
    ...
    audio_file = os.path.join(base, audio)
    x, fs = sf.read(audio_file)
    # print(x.shape)
    txt_file = os.path.join(base, audio[:-4] + '.txt')
    # print(txt_file)
    ...
    y = signal.filtfilt(b, a, x)
    wav = y * 0.95 + (prng.rand(y.shape[0]) - 0.5) * 1e-06
    D = pySTFT_len(wav, length).T
    D_mel = np.dot(D, mel_basis)
    D_db = 20 * np.log10(np.maximum(min_level, D_mel)) - 16
    S = (D_db + 100) / 100

    hop_length = (x.shape[0] - 1024) // (length-1)
