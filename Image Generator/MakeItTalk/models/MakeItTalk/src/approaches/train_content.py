...
import time
from src.dataset.audio2landmark.audio2landmark_dataset import Audio2landmark_Dataset, Audio2landmark_Dataset_array
...
class Audio2landmark_model():
    ...
    def __init__(self, opt_parser, jpg_shape=None):
        ...
        self.train_data = Audio2landmark_Dataset_array(dump_dir=opt_parser.dump_dir,
                                                dump_name='autovc_retrain_mel_std',
                                                status='train',
        ...
        self.eval_data = Audio2landmark_Dataset_array(dump_dir=opt_parser.dump_dir,
                                                 dump_name='autovc_retrain_mel_std',
                                                 status='train',
                                                 num_window_frames=opt_parser.num_window_frames,
                                                 num_window_step=opt_parser.num_window_step)
    ...
    def __train_content__(self, fls, aus, face_id, is_training=True):
        ...
        if (self.opt_parser.use_lip_weight):
            ...
            fl_loss = torch.mean(torch.abs(fl_dis_pred +face_id[0:1].detach() - fls_gt) * lip_region_w)
        else:
            # loss = self.loss_mse(fl_dis_pred + face_id, fls[:, 0, :])
            fl_loss = torch.nn.functional.l1_loss(fl_dis_pred+face_id[0:1].detach(), fls_gt)
        ...
        if (self.opt_parser.lambda_laplacian_smooth_loss > 0.0):
            ...
            loss = loss + loss_laplacian
            # wandb.log({'loss_laplacian' : loss_laplacian})
        ...
        return fl_dis_pred, face_id[0:1, :], loss, fl_loss, loss_laplacian
    ...
    def __train_pass__(self, epoch, log_loss, is_training=True):
        ...
        for i, batch in enumerate(dataloader):
            ...
            for in_batch in range(self.opt_parser.in_batch_nepoch):
                ...
                for j in range(0, inputs_fl.shape[0], seg_bs):
                    ...
                    fl_dis_pred_pos, input_face_id, loss, fl_loss, laplacian_loss = \
                        self.__train_content__(inputs_fl_segments, inputs_au_segments, input_face_id, is_training)
                    # wandb.log({'fl_loss' : fl_loss, 'laplacian_loss' : loss_laplacian})
        ...
        if (self.opt_parser.verbose <= 2):
            ...
            for key in log_loss.keys():
                ...
                # wandb.log({'Loss': log_loss[key].per('epoch')})
            print(
    ...
    def train(self):
        train_loss = {key: Record(['epoch', 'batch']) for key in ['loss', 'fl_loss', 'laplacian_loss']}
        eval_loss = {key: Record(['epoch', 'batch']) for key in ['loss', 'fl_loss', 'laplacian_loss']}
        ...
        for epoch in range(self.opt_parser.nepoch):
            ...
            # print(train_loss.keys())
            # print(eval_loss.keys())
            wandb.log({"train_total_loss": train_loss['loss'].per('epoch')})
            wandb.log({"train_fl_loss": train_loss['fl_loss'].per('epoch')})
            wandb.log({"train_la_loss": train_loss['laplacian_loss'].per('epoch')})
            
            wandb.log({"eval_total_loss": eval_loss['loss'].per('epoch')})
            wandb.log({"eval_fl_loss": eval_loss['fl_loss'].per('epoch')})
            wandb.log({"eval_la_loss": eval_loss['laplacian_loss'].per('epoch')})
